...
function placeMarker(place, myIcon, bgcolor, url) {
	for (var i = 0; i < place.length; i++) {
		var divIcon = L.divIcon({
			html: '<div class="divicon" style="background-color:' + bgcolor + '">' + place[i][0] + '</div>',
			iconSize: [0,0]
		});
		if (url) {
			var popStr = '<a href="' + place[i][3] + '" target="_blank">' + place[i][0] + '</a>';
		} else {
